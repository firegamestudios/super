...
namespace Opsive.UltimateCharacterController.AddOns.Multiplayer.PhotonPun.Game
{
    ...
    public class PunObjectPool : NetworkObjectPool, IPunPrefabPool, IOnEventCallback
    {
        ...
        private RaiseEventOptions m_MasterClientRaisedEventOptions;
        ...
        private void Awake()
        {
            ...
            m_MasterClientRaisedEventOptions = new RaiseEventOptions {
        ...
        protected override void NetworkSpawnInternal(GameObject original, GameObject instanceObject, bool sceneObject)
        {
            ...
            // Only the MasterClient can spawn objects on the network.
            if (!PhotonNetwork.IsMasterClient) {
                ...
                var data = new object[]
                {
                    index, instanceObject.transform.position, instanceObject.transform.rotation, sceneObject, spawnData
                };
                PhotonNetwork.RaiseEvent(PhotonEventIDs.ClientObjectInstantiation, data, m_MasterClientRaisedEventOptions, m_ReliableSendOptions);
            ...
            // The pooled object can optionally provide initialization spawn data.
            RaiseSpawnEvent(instanceObject, sceneObject, index, null);
        }
        ...
        /// <param name="instanceObject">The object that was instantiated from the original object.</param>
        /// <param name="sceneObject">Is the object owned by the scene? If fales it will be owned by the character.</param>
        /// <param name="sceneObject">The index of the spawned object within the pool.</param>
        /// <param name="spawnData">Any data associated with the spawn.</param>
        private void RaiseSpawnEvent(GameObject instanceObject, bool sceneObject, int index, object[] spawnData)
        {
        ...
        public void OnEvent(EventData photonEvent)
        {
            if (photonEvent.Code == PhotonEventIDs.ObjectInstantiation) {
                ...
                if (photonView != null && ObjectPool.InstantiatedWithPool(photonView.gameObject)) {
                    DestroyInternal(photonView.gameObject);
                }
            } else if (photonEvent.Code == PhotonEventIDs.ClientObjectInstantiation) {
                var data = (object[])photonEvent.CustomData;
                var instance = ObjectPool.Instantiate(m_NetworkSpawnableObjects[(int)data[0]], (Vector3)data[1], (Quaternion)data[2]);
                // Issue a callback after the object has been spawned to allow the object to be initialized.
                var spawnDataObject = instance.GetCachedComponent<ISpawnDataObject>();
                if (spawnDataObject != null) {
                    spawnDataObject.InstantiationData = (object[])data[4];
                    spawnDataObject.ObjectSpawned();
                }

                RaiseSpawnEvent(instance, (bool)data[3], (int)data[0], spawnDataObject?.InstantiationData);
            }
        }
